...
parser.add_argument('--prefix', dest='prfx', help='prefix of the fasta file')
...
with open(input_filename, 'r') as input_file:
    ...
    fieldnames = ['central_id']  # Add 'type' to the fieldnames
    data = [{field: row[field] for field in fieldnames} for row in reader]

for row in data:
    row['path'] = string_to_concatenate + '/' + prefix + row['central_id'] + suffix

with open(output_filename, 'w', newline='') as output_file:
    fieldnames = ['central_id', 'path']  # Include 'type' in the fieldnames
